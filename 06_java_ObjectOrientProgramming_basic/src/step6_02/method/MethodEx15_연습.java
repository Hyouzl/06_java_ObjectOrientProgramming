...
class NumberGame{
	...
	void printPlay() {
		...
		for (int i = 0; i < move.length; i++) {
			...
			else  				System.out.print("옷"); //move[i] 값이 8인경우 즉 0이 아닌경우 플레이어 옷으로 표시 
		}
		System.out.println();
	...
	void moveLeft() {
		//왼쪽으로 이동할 때
		if (playerLocation > 0) { //플레이어가 왼쪽으로 이동 (-1) 했을 때 인덱스 값이 0 보다 클 경우
			move[playerLocation] = 0; //이동 전 값 0으로
			move[playerLocation - 1] = 8; // 이동 후 값을 플레이어 값으로 변경
			playerLocation--;// 플레이어의 위치 변경 해주기
		}
		else {
	...
	void moveRight() {
		// 오른쪽으로 이동할 경우 
		if (playerLocation < move.length - 1) { // 플레이어가 오른쪽으로 이동 (+1) 했을 때 인덱스 값이 플레이 칸의 길이가 넘기 전 까지
			move[playerLocation] = 0; // 이동 전 값 0으로
			move[playerLocation + 1] = 8; // 이동 후 값을 플레이어 값으로 변경
			playerLocation++;// 플레이어의 위치 변경 해주기
		}
